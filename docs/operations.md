...
`granite-move-tablebase-scope` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies tablebase behavior through bounded scenario files, with conflict explanations and local-only command execution.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 5.
